...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        box, cls,cls2, mask = x.split((4, 5,6, nm), 1)

        if multi_label:
            cls_merged = torch.cat((cls, cls2), 1)
            i, j = torch.where(cls > conf_thres)
            x = torch.cat((box[i], x[i, 4 + j, None], j[:, None].float(),torch.zeros((len(j),1), dtype=float, device=x.device), mask[i]), 1)
        else:  # best class only
            conf, j = cls.max(1, keepdim=True)
            conf2, j2 = cls2.max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(),j2.float(), mask), 1)[conf.view(-1) > conf_thres]
