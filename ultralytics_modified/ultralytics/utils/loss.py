...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        pred_distri, pred_scores, pred_2 = torch.cat([xi.view(feats[0].shape[0], self.no+6, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc, 6), 1)
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()
        pred_2 = pred_2.permute(0, 2, 1).contiguous()
        ...
        # Targets
        if isinstance(batch['cls2'], list):
            batch["cls2"] = torch.cat(batch["cls2"], dim=0).cuda()
        targets = torch.cat((batch['batch_idx'].view(-1, 1), batch['cls'].view(-1, 1), batch['bboxes'], batch['cls2'].view(-1, 1)), 1)

        targets = self.preprocess(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])

        gt_labels, gt_bboxes,gt_label2 = targets.split((1, 4, 1), 2)  # cls, xyxy
        mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0)
        # Pboxes
        pred_bboxes = self.bbox_decode(anchor_points, pred_distri)  # xyxy, (b, h*w, 4)

        _, target_bboxes, target_scores, fg_mask, _, target_scores2 = self.assigner(
            pred_scores.detach().sigmoid(), (pred_bboxes.detach() * stride_tensor).type(gt_bboxes.dtype),
            anchor_points * stride_tensor, gt_labels, gt_bboxes, mask_gt, pred_2.detach().sigmoid(), gt_label2)
        ...
        print("cls", loss[1])
        loss[3] = self.bce(pred_2, target_scores2.to(dtype)).sum() / target_scores_sum2  # BCE
        print("cls2", loss[3])
        # Bbox loss
        if fg_mask.sum():
